...
def fetch_daily(ids, tipo):
    ...
    for grp in chunks(ids, 5):  # <=5 campanhas por request evita truncamento
        data = api_get(f'{ACCOUNT}/insights', {
            'level': 'campaign',
            'fields': 'campaign_id,spend,impressions,clicks,actions,results',
        ...
        for row in paginate(data):
            ...
            # resultado = coluna "Resultados" do Meta (cadastros p/ cad, visitas p/ perf)
            acc[5] += res_value(row.get('results', {}))
...
def fetch_ads(cad_ids, perf_ids):
    ...
    for d in paginate(data):
        ...
        tipo = 'perf' if d.get('campaign_id') in idset_perf else 'cad'
        res = res_value(d.get('results', {}))          # cadastros (cad) ou visitas (perf)
        spend = round(f(d.get('spend')), 2)
        cpr = cpr_value(d.get('cost_per_result', {})) or (round(spend / res, 2) if res else 0)
