...
def fetch_ads(cad_ids, perf_ids):
    ...
    for d in paginate(data):
        ...
        rows.append([
            ...
            i(d.get('reach')), res, cpr, thumbs.get(d.get('ad_id'), ''),
        ])
    log(f'  {len(rows)} anuncios')
    return rows

def fetch_thumbs():
    data = api_get(f'{ACCOUNT}/ads', {'fields': 'id,creative{thumbnail_url,image_url}', 'limit': 400})
    t = {}
    if 'error' in data: return t
    for ad in paginate(data):
        cr = ad.get('creative', {}) or {}
        t[ad['id']] = cr.get('thumbnail_url') or cr.get('image_url') or ''
    return t
