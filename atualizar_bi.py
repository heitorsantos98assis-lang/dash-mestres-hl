...
# Rendas que contam como MQL (alem de qualquer resposta que comece com "sim,")
MQL_RENDA_OK = {'ganho até r$ 10 mil por mês', 'ganho acima de r$ 10 mil por mês'}
...
def fetch_mqls():
    ...
    for row in reader:
        if len(row) <= MQL_COL_DATE:
            continue
        resp = (row[MQL_COL_RESP] or '').strip().lower()
        if not resp:
            continue
        is_mql = resp.startswith('sim,') or resp in MQL_RENDA_OK
        if not is_mql:
            continue
